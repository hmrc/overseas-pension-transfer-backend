...
class MemberIsUKResidentTransformerSpec extends AnyWordSpec with Matchers {

  val transformer = new MemberIsUKResidentTransformer

  "MemberIsUKResidentTransformer" should {

    "construct: move and convert memberDetails.memUkResident boolean to nested Yes/No string" in {
      val input = Json.obj(
    ...
    "construct: leave JSON unchanged if memUkResident is missing" in {
      val input = Json.obj(
        "memberDetails" -> Json.obj()
      )
    ...
    "deconstruct: leave JSON unchanged if memUkResident is missing in nested structure" in {
      val input = Json.obj(
        "transferringMember" -> Json.obj(
          "memberDetails" -> Json.obj(
            "memberResidencyDetails" -> Json.obj()
