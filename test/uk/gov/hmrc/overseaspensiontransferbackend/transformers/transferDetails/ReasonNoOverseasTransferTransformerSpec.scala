...
class ReasonNoOverseasTransferTransformerSpec extends AnyFreeSpec with Matchers {

  private val transformer = new ReasonNoOverseasTransferTransformer
  ...
  "ReasonNoOverseasTransferTransformerSpec" - {
    ...
    "must leave Json unchanged if transferDetails.reasonNoOverseasTransfer is missing" in {
      val inputJson = Json.obj("transferDetails" -> Json.obj())

      transformer.construct(inputJson) mustBe Right(inputJson)
    }

    "must leave Json unchanged if transferDetails.taxableOverseasTransferDetails.reasonNoOverseasTransfer is missing" in {
      val inputJson = Json.obj("transferDetails" -> Json.obj())
