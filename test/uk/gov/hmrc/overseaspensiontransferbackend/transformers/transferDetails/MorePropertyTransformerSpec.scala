...
class MorePropertyTransformerSpec extends AnyFreeSpec with Matchers {
  ...
  "MorePropertyTransformer" - {
    "must convert transferDetails.moreProp to transferDetails.typeOfAssets.moreProp boolean to nested Yes/No string" in {
      val input    = Json.obj("transferDetails" -> Json.obj("moreProp" -> true))
      val expected = Json.obj("transferDetails" -> Json.obj("typeOfAssets" -> Json.obj("moreProp" -> "Yes")))

      transformer.construct(input) mustBe Right(expected)
    }

    "must convert nested Yes/No string back to boolean" in {
      val input    = Json.obj("transferDetails" -> Json.obj("typeOfAssets" -> Json.obj("moreProp" -> "No")))
      val expected = Json.obj("transferDetails" -> Json.obj("moreProp" -> false))

      transformer.deconstruct(input) mustBe Right(expected)
    }

    "must leave JSON unchanged if moreProp is missing" in {
      val input = Json.obj("transferDetails" -> Json.obj())

      transformer.construct(input) mustBe Right(input)
    }

    "must leave JSON unchanged if transferDetails.typeOfAssets.moreProp is missing" in {
