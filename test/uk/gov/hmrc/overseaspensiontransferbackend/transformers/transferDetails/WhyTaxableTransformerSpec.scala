...
class WhyTaxableTransformerSpec extends AnyFreeSpec with Matchers {

  private val transformer = new WhyTaxableTransformer
  ...
  "ApplicableExclusionTransfromerSpec" - {
    "must move transferDetails.applicableExclusion to transferDetails.taxableOverseasTransferDetails.applicableExclusion" in {
      val inputJson = Json.obj("transferDetails" -> Json.obj("whyTaxableOT" -> "noExclusion"))
      val expected  = Json.obj("transferDetails" -> Json.obj("taxableOverseasTransferDetails" -> Json.obj("whyTaxableOT" -> "02")))
    ...
    "must move transferDetails.taxableOverseasTransferDetails.applicableExclusion to transferDetails.applicableExclusion" in {
      val inputJson = Json.obj("transferDetails" -> Json.obj("taxableOverseasTransferDetails" -> Json.obj("whyTaxableOT" -> "01")))
      val expected  = Json.obj("transferDetails" -> Json.obj("whyTaxableOT" -> "transferExceedsOTCAllowance"))
