...
class ParserHelpersSpec extends AnyFreeSpec with Matchers {
  ...
  "handleDownstreamResponse" - {
    ...
    "maps 201 Created to DownstreamSuccess" in {
      ...
      val actual   = ParserHelpers.handleDownstreamResponse[DownstreamSuccess](expected, CREATED)
    ...
    "maps 400 to HipBadRequest when responseSystemErrorType" in {
      ...
      val actual   = ParserHelpers.handleDownstreamResponse[DownstreamSuccess](expected)
    ...
    "maps 415 to UnsupportedMedia" in {
      ...
      ParserHelpers.handleDownstreamResponse[DownstreamSuccess](expected).left.toOption.get mustBe UnsupportedMedia
    }

    "maps unknown status to Unexpected" in {
      val expected = HttpResponse(IM_A_TEAPOT, "I'm a teapot")
      ParserHelpers.handleDownstreamResponse[DownstreamSuccess](expected).left.toOption.get mustBe Unexpected(IM_A_TEAPOT, "I'm a teapot")
    }
  }
}
