...
import uk.gov.hmrc.overseaspensiontransferbackend.models.transfer.{QtNumber, TransferIdentifierInvalid, TransferNumber}
...
class GetSpecificTransferHandlerSpec extends AnyFreeSpec with Matchers {
  ...
  "apply" - {
    "when qtStatus is InProgress return Right GetSaveForLaterRecord" in {
      GetSpecificTransferHandler.apply(TransferNumber("refId"), PstrNumber("12345678AB"), InProgress, None) mustBe
        Right(GetSaveForLaterRecord(TransferNumber("refId"), PstrNumber("12345678AB"), InProgress))
    }

    "when qtStatus is Submitted and versionNumber is defined" in {
      GetSpecificTransferHandler.apply(QtNumber("QT123456"), PstrNumber("12345678AB"), Submitted, Some("001")) mustBe
    ...
    "when qtStatus is Compiled and versionNumber is defined" in {
      GetSpecificTransferHandler.apply(QtNumber("QT123456"), PstrNumber("12345678AB"), Compiled, Some("001")) mustBe
